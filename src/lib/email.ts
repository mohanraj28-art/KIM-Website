...
const FROM = `"${process.env.EMAIL_FROM_NAME || 'KIP Platform'}" <${process.env.EMAIL_FROM}>`
...
<body>
  <div class="container">
    ...
    <div class="footer">
      <p>© 2025 KIP Platform. All rights reserved.</p>
...
export async function sendOtpEmail(email: string, otp: string, purpose: string = 'verification') {
    await transporter.sendMail({
        from: FROM,
        to: email,
        subject: `Your KIP verification code: ${otp}`,
        html: baseEmailTemplate(`
...
export async function sendInvitationEmail(email: string, inviterName: string, orgName: string, token: string) {
    ...
    await transporter.sendMail({
      ...
      <p><strong>${inviterName}</strong> has invited you to join <strong>${orgName}</strong> on KIP Platform.</p>
